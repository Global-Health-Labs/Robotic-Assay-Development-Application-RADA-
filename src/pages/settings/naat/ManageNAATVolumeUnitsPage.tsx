...
export default function ManageNAATVolumeUnitsPage() {
  ...
  const addMutation = useMutation({
    mutationFn: async (data: { unit: string }) => {
      const res = await axios.post('/settings/naat/volume-units', data);
      return res.data;
    },
    onSuccess: (newUnit) => {
      queryClient.setQueryData(['naat-volume-units'], (oldUnits: VolumeUnit[] = []) => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { unit: string } }) => {
      const res = await axios.put(`/settings/naat/volume-units/${id}`, data);
      return res.data;
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id: string) => {
      const res = await axios.delete(`/settings/naat/volume-units/${id}`);
      return res.data;
    },
  ...
  const handleAddVolumeUnit = () => {
    ...
    setEditState(newUnit);
  };

  const handleEdit = (unit: EditState) => {
    setEditState({
      id: unit.id,
      unit: unit.unit,
    });
  ...
  const handleSaveEdit = async () => {
    ...
    const isDuplicate = volumeUnits.some(
      (vu) => vu.unit.toLowerCase() === unitValue.toLowerCase() && vu.id !== editState.id
    );
    ...
    if (editState.isNew) {
      ...
      updateMutation.mutate({ id: editState.id, data: { unit: unitValue } });
    }
  };
  ...
  const handleDelete = (unit: EditState) => {
